...
use self::config::WidgetConfig; // Убедитесь, что путь правильный
...
impl WidgetManager {
    ...
    pub fn add_widget_with_config(
        ...
        // Получаем буфер и используем borrow_mut для мутации
        let buffer = self.buffer_manager.get_buffer();

        // Устанавливаем содержимое виджета
        let content = config.content.as_deref().unwrap_or("");
        let lines: Vec<String> = content.lines().map(String::from).collect(); // Преобразование строк в вектор
        ...
        let window = nvim_oxi::api::open_win(&buffer.borrow(), true, &float_config)?; // Здесь мы используем borrow для передачи ссылки на Buffer

        let window_opts = OptionOpts::builder().win(window.clone()).build();

        // Настройка дополнительных опций окна, если необходимо
    ...
    pub fn render_dashboard(&mut self, config: &Config) -> OxiResult<()> {
        // Получить текущие размеры терминала
        let (term_width, term_height) = get_terminal_size()?;

        // Рекурсивная функция для обхода rows и columns
        fn traverse_layout(
        ...
        fn traverse_row(
            ...
            let row = row.borrow();
            // Если у строки есть content, создаем виджет
            if let Some(content) = &row.content {
                let widget_config = WidgetConfig::new(
                    "Row Widget".to_string(),
                    row.height.clone(),
                    row.width.clone(),
                    "0%".to_string(), // По умолчанию позиция
                    "0%".to_string(), // По умолчанию позиция
                )
                ...
                // Преобразовать проценты в абсолютные значения
                ...
                // Проверить условия для отображения виджета
        ...
        fn traverse_column(
            ...
            // Если у колонки есть content, создаем виджет
            ...
            // Обход вложенных строк и колонок
    ...
    // Метод для обновления размеров и позиций виджетов при изменении размера терминала
    pub fn update_widgets(&mut self, config: &Config) -> OxiResult<()> {
        // Получить текущие размеры терминала
        let (term_width, term_height) = get_terminal_size()?;

        // Сортировать виджеты по приоритету (низкий приоритет скрывается первыми)
        ...
        for widget in &mut self.widgets {
            ...
            // Проверить условия для скрытия виджета
...
// Функция для конвертации процентов в абсолютные значения
